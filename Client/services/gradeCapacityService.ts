...
export async function getGradesByPreschool(preschoolId:number): Promise<GradeCapacity[]> {
    try {
        var token; 
        await currentToken().then((returnedTOken) => { token = returnedTOken; })
        ...
        const response = await axios.get<GradeCapacity[]>(`${BASE_URL}?preschool=${preschoolId}`, config);
