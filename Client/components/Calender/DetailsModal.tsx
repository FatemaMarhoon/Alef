                  ...
                  {dayItems.eventsForDay.map((event) => (
                    <div key={event.id} className="relative col-span-12 space-y-12 grid grid-cols-12 items-start">
                      <div className="col-span-8">
                        <div className="flex flex-col">
                          ...
                          <time className="text-xs tracki uppercase dark:text-gray-400">
                            {event.public_event ? "Public Event" : ""}
                            {event.Classes && event.Classes?.length > 0 ? 'For Classes: ' : ''}
                            {event.Classes?.map((classItem, index) => (
                              <span key={index}>{index > 0 && ', '}{classItem.class_name}</span>
                            ))}
                          </time>
                      ...
                      <div className="col-span-4 flex items-start justify-end">
                        <button className="hover:text-primary mr-1" onClick={() => handleEdit(undefined, event)}>
                          {/* <Edit /> */} Edit
                        </button>
                        <p> | </p>
                        <button className="hover:text-primary ml-1" onClick={() => onDelete(undefined, event)}>
                ...
                <div className="col-span-12 px-6 space-y-6 sm:col-span-9">
                  {dayItems.appointmentsForDay.map((appointment) => (
                    <div key={appointment.id} className="relative col-span-12 space-y-12 grid grid-cols-12 items-start">
                      ...
                      <div className="col-span-4 flex items-start justify-end">
                        <button className="hover:text-primary mr-1" onClick={() => handleEdit(appointment, undefined)}>
                          {/* <Edit /> */} Edit
                        </button>
                        <p> | </p>
                        <button className="hover:text-primary ml-1" onClick={() => onDelete(appointment, undefined)}>
                          {/* <Delete color="error" /> */}
                          Delete
                        </button>
                      </div>
